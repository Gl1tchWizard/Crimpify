...
- **Service worker:** cachenaam is `crimpify-v59`. Bumpen bij elke deploy die
  bestanden wijzigt (`crimpify-v60`, enz.), anders zien bezoekers de oude versie.
...
2. **Sociale maat = completions, geen likes.** Kaarten tonen voltooiingen in de
   ...
   Echt tellen vraagt een backend. De mock-tellers zijn verwijderd (v0.48):
   geen done-veld tot er echte completions zijn, geen placeholder, geen nul;
   de kaartweergave verbergt de teller volledig als het veld ontbreekt.
...
4. **Planken, hybride strategie tot er een backend is.**
   ...
   - "Coach sessions" (v0.48): vaste plank met de echt ontworpen sessies,
     gevoed door het veld `designed: true` op MOCK_CHOOSE-entries. Dat veld
     ...
     zelf heeft aangeleverd. Vijf eigen lm-blokken, precies de vijf secties
     die Jaap zelf aanleverde (warm-up, finger training, gym boulders,
     kilterboard, core); zijn witregels zijn de blokgrenzen. Bewust geen
     bibliotheekblokken erin: de Charlie warm-up en het C4HP-curlblok
     stonden er eerst in en lazen als een voorschrift van Jaap. Limit heeft als enige designed-sessie wel een
...
12. **Echte completions.** De mock-tellers zijn verwijderd (v0.48); het
    done-veld komt pas terug wanneer een completion echt telt, bij een
    afgemaakte sessie via een backend. Geen nieuwe nepgetallen tot die tijd.
...
- Versienummer op de splash (nu v0.64) bij elke release ophogen, samen met de sw-cache.
