...
const BLOCKLIB = {
  ...
  easyTen: { n:'10 easy boulders', t:15, tMin:10, tMax:20, c:'var(--prepare)', rpe:'3-4', grade:'easy', checklist:true, target:10, range:'10',
  ...
  mediumTwenty: { n:'20 medium boulders', t:35, tMin:25, tMax:45, c:'var(--volume)', rpe:'5-6', grade:'medium', checklist:true, target:20, range:'20',
  ...
  peFlow: { n:'PE Flow', t:42, tMin:30, tMax:60, c:'var(--volume)', rpe:'6-8',
  ...
  // ── performance cores ──
  compStyle: { n:'Comp blocks', t:50, tMin:40, tMax:70, c:'var(--volume)', rpe:'8-9', sets:6, rest:5,
  ...
  // ── recovery cores ──
  easyClimb: { n:'Easy climbing', t:40, tMin:20, tMax:60, c:'var(--prepare)', rpe:'3-4',
  ...
  // ── skill cores ──
  drillBlocks: { n:'Skill blocks', t:45, tMin:30, tMax:60, c:'var(--skill)', rpe:'5-6', sets:3, rest:2, drills:true,
  ...
  slab: { n:'Slab (if energy allows)', t:12, tMin:8, tMax:20, c:'var(--skill)', rpe:'5',
    why:'Easy slab as a technical dessert. Balance and footwork. Skip without guilt if you are empty — walk away fresh.' },
  skillLight: { n:'Movement skill light', t:12, tMin:8, tMax:20, c:'var(--skill)', rpe:'3-4', drills:true,
    why:'One skill of your choice on easy terrain. Pure coordination, zero strength. Cool the nervous system down.' },
  drillsOnly: { n:'Movement skills', t:20, tMin:15, tMax:30, c:'var(--skill)', rpe:'4-5', drills:true,
  ...
  drillLibrary: { n:'Skill library', t:30, tMin:20, tMax:60, c:'var(--skill)', rpe:'4-5', library:true,
    why:'The full library. Pick the skills you want to do today yourself.' },
  sprayLight: { n:'Spray wall (short)', t:15, tMin:10, tMax:25, c:'var(--volume)', rpe:'4-5',
  ...
  frontGrowth: { n:'Limit work (fresh)', t:20, tMin:15, tMax:45, c:'var(--max-effort)', rpe:'8-9',
  ...
  stretchLong: { n:'Extended stretch', t:25, tMin:15, tMax:35, c:'var(--prepare)', rpe:'-',
  ...
  // Base Camp (90 min: 15+15+50+10, sessiebereik 75-120)
  bcWarmup: { n:'Progressive Warm-up', t:15, tMin:10, tMax:20, c:'var(--prepare)', rpe:'2-4', sessionOnly:true,
    why:'Six to ten progressively harder boulders across different angles and grip types. Begin very easily, gradually approach the intensity of the main session, and finish warm and coordinated, not pumped. The first easy boulders of the session also contribute to warming up: avoid turning the warm-up into a separate thirty minute workout.' },
  bcBoardPrimer: { n:'Board Movement Primer', t:15, tMin:10, tMax:20, c:'var(--skill)', rpe:'4-5', sessionOnly:true,
    why:'Body tension, foot pressure and controlled movement while still fresh, on a system board, spray wall or steep gym wall. Climb about four to six manageable boulders around rpe 4 to 5. Focus on controlled feet, body tension, relaxed grip, steady rhythm and clean execution.\nNo projecting, no benchmark chasing, no repeated failure; at most one or two clean ascents per problem. Lower the angle or difficulty if movement quality is compromised. This is a movement primer, not a board power session. No suitable board or wall available? Replace this block with controlled steep-wall technique climbing.' },
  bcMileage: { n:'Quality Mileage', t:50, tMin:40, tMax:70, c:'var(--volume)', rpe:'3-6', sessionOnly:true, checklist:true, target:23, range:'18-28',
    why:'A large amount of controlled, varied climbing with technique intact. Aim for roughly 18 to 28 boulders depending on wall density, boulder length, experience, gym traffic and current capacity. Keep 70 to 80 percent easy: immediately climbable, technically clean, around rpe 3 to 5, done in one attempt. The rest may take more attention, reach about rpe 6, or need a second attempt.\nRules: maximum two attempts per problem, no projects, no repeated hard failures, avoid sustained heavy pump, and cover at least three different wall angles or movement styles. Continue only while movement remains controlled and stop counting boulders when quality breaks. Easier climbing still counts; forced climbing does not.' },
  bcWindDown: { n:'Easy Wind-Down', t:10, tMin:5, tMax:15, c:'var(--prepare)', rpe:'1-2', sessionOnly:true,
    why:'Gradually reduce intensity: very easy climbing, easy traversing, walking, relaxed mobility, breathing and shaking out. Finish feeling better than an abrupt stop would.\nScale the session down when feet repeatedly slip, grip pressure becomes excessive, easy boulders begin feeling hard, two consecutive problems need more than two attempts, pump stops recovering normally, or fingers, elbows or shoulders become uncomfortable. A successful Base Camp ends with the feeling that more climbing was possible, while the intended quality work is complete.\nReflection: where did you move most efficiently today?' },
  // Board Blitz (120 min: 10+40+60+10, sessiebereik 90-150; blokgrenzen
  // 30-50 en 50-75 komen letterlijk uit de brief)
  bbPrimer: { n:'Power Primer', t:10, tMin:8, tMax:15, c:'var(--prepare)', rpe:'3-5', sessionOnly:true,
    why:'Short and non-fatiguing: a pulse raiser, dynamic mobility for wrists, shoulders, trunk and hips, scapular activation, Active Finger Curls and light pulling recruitment. The finger curls prepare recruitment here; they are not a full finger strength session.' },
  bbReady: { n:'Board Ready', t:40, tMin:30, tMax:50, c:'var(--max-effort)', rpe:'4-8', sessionOnly:true,
    why:'One continuous ramp to genuine board readiness, using only as many boulders as you need. Example for a climber targeting three 7A+ problems: several grade 5 boulders, one or two 6As, one or two 6Bs, one 6C, and at most one known 7A as a final readiness check. That is an example, not a mandatory grade ladder: warm up faster with fewer steps, or take more time with cold fingers, a steep angle or a slow readiness response. The final known hard boulder should be familiar, require meaningful force, need no beta solving, act purely as a readiness check and be attempted once at most.\nGreen light: hard pulls feel fast, fingers and shoulders feel prepared, body tension appears quickly, the known boulder feels demanding but available, and there is no pain or unexpected stiffness. Orange light: movement feels slow, the known boulder climbs badly or hard positions feel unusually difficult; rest longer, add one moderate known boulder, or select easier targets. Red light: pain, unsafe grip, clear lack of power or the readiness boulder feels like a project; scale the targets down or change the session.\nRule: warm up until hard climbing feels available, not until the clock says go.' },
  bbThree: { n:'Three by Three', t:60, tMin:50, tMax:75, c:'var(--max-effort)', rpe:'8-9', sessionOnly:true,
    why:'Three hard board boulders, maximum three fully recovered working attempts each, chosen one at a time: pick Boulder One and complete its attempt loop, then Boulder Two in a clearly different style, then Boulder Three with another meaningful contrast (static tension versus dynamic power, compression versus pulling, horizontal versus vertical, a different angle, poor feet versus foot drive, another grip or movement family). Do not select all three in advance and do not force variation purely to tick a category.\nA good target feels like: I can probably climb this today in one to three fully recovered attempts. It requires full attention, has readable or mostly known beta, creates real uncertainty and offers a realistic chance of success. Avoid complete limit projects, coordination lotteries, long power endurance problems, barely possible starts, and anything that needs ten attempts to understand.\nAttempt 1: read and commit. Attempt 2: change the most important detail. Attempt 3: execute the best available solution. Rest four to seven minutes, longer if not fully recovered. No bonus attempts and no half attempts in between.\nFirst-try rule: a first-try send still counts as a target when it required full commitment at a small margin, around rpe 8 to 9, clearly harder than a warm-up boulder. Ask yourself: how hard was that? Hard, it counted: the boulder stands as one of the three. Controlled, probably too easy: log it as a final primer and choose one harder replacement in the same style, with at most one upgrade per slot. Do not keep raising difficulty until failure; failure is not the proof that difficulty was correct.\nStop or reduce difficulty when two consecutive attempts clearly lose speed, catches become uncontrolled, fingers, elbows or shoulders hurt, skin changes how you use the holds, full commitment is gone, or you start adding unplanned attempts. All three comfortable? Raise the challenge slightly next Board Blitz instead of adding volume now.' },
  bbLog: { n:'Log & Downshift', t:10, tMin:5, tMax:15, c:'var(--prepare)', rpe:'1-2', sessionOnly:true,
    why:'Finish without adding bonus projects: log attempts and sends, note which style felt strongest and whether the target difficulty was appropriate. Do not add an extra project because the session ended well.\nReflection: which problem produced your best power, and what should you repeat next time?' },
  // Flow State (75 min: 15+10+30+15+5, sessiebereik 60-90)
  fsWarmup: { n:'Progressive Warm-up', t:15, tMin:10, tMax:20, c:'var(--prepare)', rpe:'2-4', sessionOnly:true,
    why:'Climb six to ten easy boulders across different wall angles, gradually increasing movement complexity. Keep breathing relaxed and finish warm without accumulating pump.' },
  fsPrimer: { n:'Flow Primer', t:10, tMin:5, tMax:15, c:'var(--skill)', rpe:'3-4', sessionOnly:true,
    why:'An immediate comparison between ordinary climbing and connected climbing. Choose two or three very easy boulders and climb each twice. First ascent: climb normally. Second ascent: try to make it feel quieter, smoother and easier without deliberately climbing slower. Reduce unnecessary pauses, repeated hand adjustments, foot searching, excessive grip tension and movement that does not contribute to the next position.\nPrompt: can the second ascent feel easier without becoming slower?' },
  fsConnect: { n:'Connect the Moves', t:30, tMin:20, tMax:40, c:'var(--skill)', rpe:'3-5', sessionOnly:true,
    why:'Three rounds of about ten minutes. Per round: three to five easy or moderate boulders, a maximum of two attempts each, mostly around rpe 3 to 5. Change to an easier problem when difficulty prevents attention to movement.\nRound 1, See Ahead: read the full boulder before starting, identify the key foot and hip positions, understand roughly the first three movements, and start with a movement plan rather than reacting one hold at a time.\nRound 2, Move Through: let each position prepare the next, avoid unnecessary stops and repeated grip adjustments, use momentum where appropriate, and maintain rhythm without rushing.\nRound 3, Breathe and Soften: continue breathing during difficult movement, relax the jaw, free hand and shoulders where possible, use tension only where it contributes, and notice where unnecessary tension interrupts flow.' },
  fsFreeFlow: { n:'Free Flow', t:15, tMin:10, tMax:20, c:'var(--skill)', rpe:'3-5', sessionOnly:true,
  ...
  // The Grinder (105 min: 20+15+60+10, sessiebereik 90-120). HEHE is de
  // enige hoofdprikkel: geen 4x4 erachteraan.
  tgWarmup: { n:'Power-Endurance Warm-up', t:20, tMin:15, tMax:25, c:'var(--prepare)', rpe:'2-5', sessionOnly:true,
    why:'Prepare for repeated overhanging climbing without creating meaningful pump before the main work: begin with easy general movement, progress through easy climbing, include some overhanging climbing, gradually increase the length and intensity of boulders, and finish with one moderately sustained problem. You should feel warm and ready, not fatigued.' },
  tgSetup: { n:'HEHE Setup', t:15, tMin:10, tMax:20, c:'var(--volume)', rpe:'4-6', sessionOnly:true,
    why:'Choose and verify the four boulders before the sets begin: Hard A, Easy A, Hard B, Easy B. Use the same four boulders in every set and do not choose new ones between sets.\nHard boulders: overhanging, at least about eight moves, climbed before but not as an effortless first try, normally climbable yet still creating real uncertainty today, familiar enough that beta is not the primary challenge, and continuous rather than dominated by one isolated limit move. The sweet spot: I know I can climb this, but I am not fully certain I can climb it four times today. Do not choose a current project, an unreliable boulder, a very short limit boulder, a coordination lottery, or anything with one move that frequently fails when fresh.\nEasy boulders: as easy as practically possible, overhanging, about six to eight moves, large comfortable holds, fully known movement, no genuinely hard move, and essentially impossible to fall from. They should feel almost absurdly easy when fresh: they are active recovery that keeps you on the overhang and in the forearms without adding pump.\nBefore set one: confirm both hard boulders are familiar, confirm both easy boulders are genuinely easy, avoid accumulating unnecessary attempts, and rest fully.' },
  tgHehe: { n:'The Grinder, HEHE', t:60, tMin:45, tMax:75, c:'var(--volume)', rpe:'8-9', sessionOnly:true,
  ...
  // ── Limit (Jaap dJ, sept 2026). Vijf blokken, één per sectie van Jaaps
  //    eigen indeling: warm-up, finger training, gym boulders, kilterboard,
  //    core. 22+23+12+52+6 = 115 min, sessiebereik 97-160. Alles in zijn
  //    woorden; geen bibliotheekblok met een eigen naam of andermans
  //    protocol in een coach-sessie.
  lmWarmup: { n:'Warm-up', t:22, c:'var(--prepare)', rpe:'2-4', guided:true, fixed:true, sessionOnly:true,
    why:'One warm-up, five parts, in this order: lower body, fingers, shoulders, pull, lock-off. Lower body first with dynamic stretching through the ranges you are about to use. Then finger curls, building gently, nothing near a max. Then hanging shoulder shrugs, ten calm reps with straight arms and the shoulder blades pulled down and together. Then two sets of two to five pull-ups, supported by a band or a foot on a box if that is what it takes to keep them clean. Finish with two ten second two-arm lock-offs at ninety degrees. Rest between every set. Nothing here should feel hard; the finger lifts are next and they need you fresh.',
    items:[
      { n:'Lower body: dynamic stretching', note:'hips, groin, hamstrings, calves, ankles', sec:420 },
      { n:'Fingers: finger curls', note:'build gently, stay well off max', sec:300 },
      { n:'Shake out', note:'breathe', sec:30, rest:true },
      { n:'Shoulders: hanging shoulder shrugs', note:'10 reps, straight arms, blades down and together', sec:60 },
      { n:'Rest', note:'arms down', sec:30, rest:true },
      { n:'Pull-ups, set 1', note:'2 to 5 reps, band or foot support if needed', sec:45 },
      { n:'Rest', note:'full recovery, this is prep', sec:60, rest:true },
      { n:'Pull-ups, set 2', note:'2 to 5 reps, same support', sec:45 },
      { n:'Rest', note:'shake the arms out', sec:60, rest:true },
      { n:'Lock-off, set 1', note:'two arms, hold 10 seconds at 90 degrees', sec:20 },
      { n:'Rest', note:'arms down', sec:60, rest:true },
      { n:'Lock-off, set 2', note:'two arms, hold 10 seconds at 90 degrees', sec:20 },
  ...
  lmBoard: { n:'Kilterboard', t:52, tMin:40, tMax:75, c:'var(--max-effort)', rpe:'8-10', sessionOnly:true,
    why:'Kilterboard, or the board you have, in three phases. Start with an easy boulder and increase the difficulty every climb until you hit your max flash level; rest properly from the middle of the ramp onwards so every attempt is a real flash attempt.\nThen pick one or two boulders at a grade you have not climbed before, or at your max grade, and take ten to fifteen minutes per boulder to project. Work the moves, find the beta, then link. Rest fully between real attempts.\nThen keep climbing boulders two grades below your max, no more than five minutes per boulder. The session ends when you lose power and start to get tired. That end is the plan, not a failure: log the stoplight and go home. The first boulder that feels heavy for its grade is your signal. Finish that one and stop.' },
  lmCore: { n:'Core', t:6, tMin:5, tMax:8, c:'var(--prepare)', rpe:'7-8', sets:2, rest:2, sessionOnly:true,
    why:'Toes to bar, two rounds of as many clean reps as you can, two minutes rest between rounds. Hang with straight arms, bring the toes to the bar with control and lower without swinging. Too hard today? Knee raises or straight leg raises count just the same; pick the version where you can still control the way down. Stop the round when the swing takes over.' },
...
// elders de slider, en bij ∞ de som van de huidige blokduren
function getTFinite() {
  if (activeSessionId === 'custom') return currentBlocks.reduce((s,b)=>s+b.t,0);
  return isFinite(getT()) ? getT() : currentBlocks.reduce((s,b)=>s+b.t,0);
}
...
// duurmodel per blok: [min, max] waarbinnen de fitter en de steppers bewegen.
// vast (fixed/dynamic) = min == max == t; ontbrekende velden krijgen een
// conservatieve fallback (toekomstige/geimporteerde blokken)
function blockBounds(b) {
  if (b.fixed === true || b._key === 'dynamic') return { min: b.t, max: b.t };
  const min = b.tMin != null ? b.tMin : Math.max(3, Math.round(b.t * 0.5));
  const max = b.tMax != null ? b.tMax : Math.max(b.t, Math.round(b.t * 1.5));
  return { min, max };
}
function clampToBounds(b, minutes) {
...
function fitToBudget(base, id) {
  ...
  const items = base.map(b => {
    const ov = ovs[b._slot];
    if (ov != null) return { b, t: clampToBounds(b, ov), fixed: true };
    if (b.fixed === true || b._key === 'dynamic') return { b, t: b.t, fixed: true };
    return { b, t: b.t, fixed: false };
  });
  ...
  if (flex.length && isFinite(budget)) {
    if (conflict) {
      ...
      flex.forEach(x => { x.t = blockBounds(x.b).min; });
    } else {
      // water-filling: schaal vanaf de basis, klem wie buiten zijn model valt
      // en herverdeel de rest tot niemand meer klemt
      let avail = budget - fixedTotal;
      let pool = flex.slice();
      let guard = 0;
      while (pool.length && guard++ < 30) {
        const baseSum = pool.reduce((s,x)=>s+x.b.t,0);
        const r = avail / baseSum;
        const clamped = pool.filter(x => { const bd = blockBounds(x.b); return x.b.t*r < bd.min || x.b.t*r > bd.max; });
        if (!clamped.length) { pool.forEach(x => { x.t = Math.round(x.b.t * r); }); break; }
        clamped.forEach(x => {
          const bd = blockBounds(x.b);
          x.t = x.b.t*r < bd.min ? bd.min : bd.max;
          avail -= x.t;
        });
...
function adjustBlock(id, slotIdx, currentT, delta) {
  const b = currentBlocks[slotIdx];
  if (!b) return;
  const bd = blockBounds(b);
  if (bd.min === bd.max) return;   // vast blok: niets te draaien
  if (!durationOverride[id]) durationOverride[id] = {};
  durationOverride[id][slotIdx] = Math.min(bd.max, Math.max(bd.min, currentT + delta));
  buildSlab();
  renderPreview();
}
...
function openDraft() {
  ...
  durationOverride['custom'] = (d.ov && typeof d.ov === 'object') ? d.ov : {};
  activeSessionId = 'custom';
...
function openFav(i) {
  ...
  // bewaarde blokduren terugzetten (additief veld; oude favs vallen op basis terug)
  durationOverride['custom'] = {};
  if (Array.isArray(f.d)) f.d.forEach((mv, i) => { if (typeof mv === 'number') durationOverride['custom'][i] = mv; });
  activeSessionId = 'custom';
...
function ensureDraftMode() {
  ...
  // gefitte duren meenemen: wat je zag is wat je bewerkt (som-leidende builder)
  durationOverride['custom'] = {};
  currentBlocks.forEach((b, i) => { durationOverride['custom'][i] = b.t; });
  activeSessionId = 'custom';
  sessionLocked = false;
}
...
const BLOCK_GROUPS = [
  { name:'Warm-up & activation',        keys:['dynamic','warmup','warmupFinger','gymWarmup','mobilityOpen','tensionAct','easyTen','noHangsEmil','tendonClimb','tendonFull','fiveWarmup','wallRamp','ownWarmup','activeCurls','phWarmup','bcWarmup','bbPrimer','fsWarmup','nfWarmup','tgWarmup','sdWarmup','edMobility','fgWarmup','csWarmup','trMobility','lmWarmup'] },
  { name:'Technique & skills',          keys:['drillsOnly','drillBlocks','drillLibrary','skillLight','slab','boardApply','skillChoice','slabWork','cleanRepeat','bcBoardPrimer','fsPrimer','fsConnect','fsFreeFlow','nfDrill','nfReal','nfNormal','lmGym'] },
  ...
  { name:'Max strength & power',         keys:['limitBlocks','project','board1','campus','dynos','pyramide','frontGrowth','lockoffs','pullStrength','fourShots','phDynamic','phLimit','bbReady','bbThree','sdProject','csRound','csPressure','lmBoard'] },
  { name:'Finger strength',               keys:['maxHangs','progDeadhangs','fgPrep','fgMax','lmFingers'] },
...
const UX_COLOR = 'var(--graphite)';  // eigen oefeningen zonder categorie; met gekozen categorie (g) volgt de groepskleur, YOURS-badge blijft de herkomst
function applyCategoryColors() {
  ...
  Object.keys(BLOCKLIB).filter(k => k.startsWith('ux_')).forEach(k => {
    const b = BLOCKLIB[k];
    b.c = (b.g && CAT_COLOR[b.g]) ? CAT_COLOR[b.g] : UX_COLOR;
  });
}
...
function openNewExercise(editKey) {
  ...
  // categoriekeuze bepaalt de kleur; bestaande blokken zonder categorie
  // kiezen hem alsnog bij het bewerken
  const sel = document.getElementById('neCat');
  sel.innerHTML = `<option value="" disabled ${b && b.g ? '' : 'selected'}>What does it train? Choose a category</option>` +
    BLOCK_GROUPS.map(gr => `<option value="${gr.name}" ${b && b.g === gr.name ? 'selected' : ''}>${gr.name}</option>`).join('');
...
function encodePayload(name, keys, time, color, durations, meta) {
  ...
  if (own.length) {
    payload.x = {};
    own.forEach(k=>{ const b = BLOCKLIB[k]; payload.x[k] = { n:b.n, t:b.t, rpe:b.rpe, why:b.why, links:b.links, g:b.g }; });
  }
...
function importFromHash() {
  ...
  if (p.x) {
    ...
    Object.keys(p.x).forEach(k=>{
      const d = p.x[k];
      const grp = (d.g && CAT_COLOR[d.g]) ? d.g : null;
      const block = { n:d.n||'Exercise', t:d.t||10, c: grp ? CAT_COLOR[grp] : UX_COLOR, rpe:d.rpe||'–', why:d.why||'', fixed:true };
      if (grp) block.g = grp;
  ...
  // keys filteren met behoud van de bijbehorende duur-indices (d is additief)
  const validKeys = [], validDur = [];
  p.k.forEach((k, i) => {
    if (!BLOCKLIB[k]) return;
    validKeys.push(k);
...
function blockGroupName(key) {
  if (key && key.startsWith('ux_')) return (BLOCKLIB[key] && BLOCKLIB[key].g) || 'Own';
...
const MOCK_CHOOSE = [
  ...
  { cat:'featured', name:'Base Camp',      coach:'Ana Kovač',     designed:true, mins:90, color:'green',  rpe:'3-6', load:2, sys:'capacity', goal:'Aerobic base', gear:['Gym wall','Kilterboard'], level:'all levels', keys:['bcWarmup','bcBoardPrimer','bcMileage','bcWindDown'],
    intent:'Volume day. Stay smooth, stop before form breaks.',
    why:'A strong aerobic base helps you recover between efforts and accumulate more quality climbing across the week. Start with controlled board movement, then build mileage across different angles and styles. Stay smooth, keep the pump manageable, and stop before form breaks.' },
  { cat:'new',      name:'Board Blitz',    coach:'Teo Marchetti', designed:true, mins:120, color:'amber',  rpe:'8-9', load:4, sys:'power', goal:'Max power', gear:['Kilterboard'], level:'intermediate+', keys:['bbPrimer','bbReady','bbThree','bbLog'],
  ...
  { cat:'coach', name:'Limit', coach:'Jaap dJ', designed:true, mins:115, color:'amber', rpe:'8-9', load:4, sys:'strength', goal:'Finger limit', gear:['Kilterboard','Weights','Gym wall'], level:'intermediate+', keys:['lmWarmup','lmFingers','lmGym','lmBoard','lmCore'], addedDate:'2026-09-07',
...
function blockGroupName(key) {
  if (key.startsWith('ux_')) return (BLOCKLIB[key] && BLOCKLIB[key].g) || 'Own';
